...
(
  ...
  INDEX idxProject
    (user_id),
  CONSTRAINT user_project FOREIGN KEY
    (user_id) REFERENCES user_reg
    (user_id) ON
    DELETE CASCADE ON
    UPDATE CASCADE
) ENGINE = InnoDB;
  ...
  PRIMARY KEY
      (task_id),
  INDEX idxTaskProject
      (proj_id),
  INDEX idxTaskUser
      (user_id),
  CONSTRAINT project_task FOREIGN KEY
      (proj_id) REFERENCES project
      (proj_id) ON
      DELETE CASCADE ON
      UPDATE CASCADE,
  CONSTRAINT user_task FOREIGN KEY
      (user_id) REFERENCES user_reg
      (user_id) ON
      DELETE CASCADE ON
      UPDATE CASCADE
